...
export const Card = ({ card, open, onOpen, className = "", openWidth, closedWidth }: Props) => {
	...
	return (
		...
		>
			...
			>
				<div
					className={`relative transition-opacity ${open ? "z-10 opacity-100" : "z-0 opacity-0"}`}
				>
					{card.content}
				</div>
				<button
					onClick={onOpen}
					className={`absolute left-0 top-0 flex h-full cursor-pointer flex-col items-start justify-between transition-opacity ${open ? "z-0 opacity-0" : "z-10 opacity-100"}`}
					style={{ width: `${closedWidth}px` }}
				>
					<Text className="w-full px-9 py-10 text-left text-3xl font-normal text-gray-900 md:px-11 md:py-12">
						{card.title}
					</Text>
