...
export const AnimatedMultipleCircles = ({ className = "" }: Props) => {
	...
	// 		if (circle1Ref.current && orbitingObject1Ref.current) {
	// 			const rect = circle1Ref.current.getBoundingClientRect();
	// 			orbitingObject1Ref.current.style.left = `${rect.left}px`;
	// 			orbitingObject1Ref.current.style.top = `${rect.top}px`;
	// 		}
	// 	};
	...
	return (
		...
		>
			...
			<path
				...
				stroke="#624049"
			/>
			{/* <div className="absolute left-[23%] top-0 opacity-0" ref={orbitingObject1Ref}>
				<Star className="w-5" />
			</div> */}
		</svg>
	);
